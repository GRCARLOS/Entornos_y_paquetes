...
def steper(self,I_error, E_integral,h,x_sol, Tl, I_reference,error_umbral, action): # Conf. provisional p/desarrollo de la sulución
    ...
    ## x_sol_next[0]= corriente, x_sol_next[1]=poscición , x_sol_next[0]= velocidad

    ## Si la corriente estimada es mayor que 3.9A o "nan", salimos del bucle for.
    if x_sol_next[0][0]>3.9 or np.isnan(x_sol_next[0][0]):
        terminated=False
        truncated=True
        self._agent_error_percentual=100
        reward= -1
    else:
        self._agent_error_porcentual = abs((I_reference-x_sol_next[0][0])/I_reference)*100
